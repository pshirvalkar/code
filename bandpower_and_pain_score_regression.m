...
for b=1:6 % for each power band
    ...
    %ACC
eval(['LFPspectra.accbandpower{b} = mean(ACCspectra(' bands{b} ',:));']);
figure(1)
subplot(3,2,b)
hold all
...
%     Y=zscore(Y); %use this if you want to zscore the Pain scores
    ...
    text(max(LFPspectra.accbandpower{b}),8,{'R^2 =' num2str(ACCbandSTATS(b,1)),'p = ' num2str(ACCbandSTATS(b,3))}); %R^2 is first value, p is 3rd
scatter(x1,Y,150,'.');
title(bands{b});ylabel('Pain Score');xlabel('Power')
    
    %OFC
eval(['LFPspectra.ofcbandpower{b} = mean(OFCspectra(' bands{b} ',:));']);
figure(2)
subplot(3,2,b)
hold all
    %regress and plot line
